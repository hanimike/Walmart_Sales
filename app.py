...
st.write(f"Answer: There are {unique_stores} unique stores in the dataset.")

st.header("Question 2 :  What is the date range covered by the dataset")
date_range_start = data['Date'].min() 
date_range_end = data['Date'].max() 
st.write(f"Answer: The dataset covers from {date_range_start.date()} to {date_range_end.date()}")
...
st.write(f"Answer: Store {max_sales_store} had the highest total weekly sales of ${max_sales_value:,.2f}")

st.subheader("Question 4: What is the average weekly sales for holiday weeks vs non-holiday weeks? ")
holiday_sales = data.groupby('Holiday_Flag')['Weekly_Sales'].mean() 
st.write(f"Answer: Average weekly sales for non-holiday weeks: ${holiday_sales[0]:,.2f}") 
...
st.write(f"Answer: Store {max_avg_store} had the highest average weekly sales of ${max_avg_value:,.2f}") 

st.subheader("Question 6: Calculate the correlation between Temperature and Weekly_Sales for each store separately")
correlations = {} 
...
st.subheader("Question 7: For each store, find the month with the highest average sales")
...
st.subheader("Question 8: What is the average Fuel_Price during holiday weeks compared to non-holiday weeks")
...
st.header("Which store has the highest average unemployment rate")
...
st.header(" Question 10: Find the week with the maximum Weekly_Sales for each store") 
...
st.header(" Question 11: Calculate the total sales for each year in the dataset")
...
st.subheader(" Question 12: For each store, find the quarter with the highest average sales")
...
st.subheader("Question 13: What is the average CPI value for weeks where Weekly_Sales were above the overall median")
store_quarter_sales = data.groupby(['Store', 'Quarter'])['Weekly_Sales'].mean().reset_index()
idx_q = store_quarter_sales.groupby('Store')['Weekly_Sales'].idxmax() 
best_quarters = store_quarter_sales.loc[idx_q]
st.subheader("Best Quarter by Store (Average Weekly Sales)") 
...
st.subheader("Question 14: Which store had the most consistent weekly sales (lowest standard deviation)")
...
st.subheader("Question: Store Sales Consistency") 
